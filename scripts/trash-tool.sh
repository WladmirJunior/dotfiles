...
trash_restore() {
  ...
  while IFS= read -r line; do
    [ -n "$line" ] || continue
    # MANIFEST lines read "origin -> stored". Split on the LAST " -> ": the
    # stored path is generated by the installer and never contains that
    # sequence, while a (pathological) origin might.
    origin="${line% -> *}"
    stored="${line##* -> }"
